...
- The plugin itself lives inside dsh and therefore **cannot** restart it, so this
  ...
  in [`service/`](service/README.md) (systemd / launchd / Windows scheduled task).
...
- The four commands shown and editable in the settings card (start / stop /
  ...
  run. **Stopping means stopped**: `autoRevive` defaults to `false`; tick it only
  if a crashed dsh should come back on its own.
