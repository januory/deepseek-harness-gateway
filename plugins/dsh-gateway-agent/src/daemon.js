...
/**
 * Readiness budget after `start`: spawning the child (or running the start
 * script) only means "a process exists". dsh still has to boot, bind its port
 * and reconnect as a console node — until then the gateway can neither relay
 * the console nor answer the relay at all. While we wait we report `starting`,
 * so the portal shows 处理中 instead of claiming 运行中 for a machine that is
 * not reachable yet. 120 × 500ms = 60s; a healthy dsh leaves the loop as soon
 * as its status probe passes.
 */
...
export class Supervisor {
  constructor({ dir, store, logger }) {
    ...
    /** Bumped by every start/stop so a stale readiness wait cannot win. */
    this.startToken = 0
    this.backoffMs = RECONNECT_BASE_MS
    this.heartbeat = null
    /** The heartbeat sender, kept so setState can push a transition at once. */
    this.beat = null
    this.watchdog = null
  ...
  setState(state, error) {
    ...
    this.persistState()
    // Push the transition now: the periodic heartbeat is 15s apart, which would
    // make 启动中/运行中/已关闭 look frozen for most of a start.
    this.heartbeatNow()
  ...
  async start() {
    ...
    if (this.config.child.command) {
      ...
      // Spawned ≠ ready. Answer `starting` now and let readiness arrive as a
      // heartbeat: a cold dsh boot can outlast the gateway's 12s control
      // timeout, so blocking here would surface as "daemon start timed out".
      this.armReadiness(token)
      return this.state
    }
    ...
    this.startedAt = new Date().toISOString()
    // Same as child mode: the script usually backgrounds dsh, so wait for the
    // probe in the background and let the state transition arrive as a push.
    this.armReadiness(token)
    return this.state
  }

  /** Kick off the background readiness wait; a superseded action cancels it. */
  armReadiness(token) {
    void this.waitReady(token).catch((e) => this.log.error(`readiness check failed: ${e && e.message ? e.message : e}`))
  }

  /**
   * Wait until dsh is actually up, then report `running`; report `exited` when
   * it never answers or the child dies first. Never throws — it runs detached
   * from the control request, and state is the only thing the gateway reads.
   *
   * "Up" means the configured status probe passes (the test topology probes the
   * dsh port). With no probe there is nothing to wait for: a live child counts
   * as running, and script mode keeps its bookkeeping-only behaviour.
   */
  async waitReady(token) {
    const childMode = !!this.config.child.command
    const hasProbe = !!(this.config.scripts && this.config.scripts.status && String(this.config.scripts.status).trim())
    const current = () => token === undefined || token === this.startToken
    if (!hasProbe) {
      if (!childMode || this.isChildRunning()) this.setState(DaemonState.RUNNING, null)
      return this.state
    }
    for (let i = 0; i < READY_ATTEMPTS; i++) {
      await sleep(READY_INTERVAL_MS)
      if (!current()) return this.state // a newer start/stop owns the state now
      if (childMode && !this.isChildRunning()) return this.state // onChildExit recorded the reason
      if ((await probeRunning(this.config.scripts, this.vars(), this.config.child.cwd)) === true) {
        this.setState(DaemonState.RUNNING, null)
        return this.state
      }
    }
    if (current()) this.setState(DaemonState.EXITED, `dsh did not come up within ${READY_ATTEMPTS * READY_INTERVAL_MS}ms`)
    return this.state
  }
  ...
  async stop() {
    this.reloadConfig()
    // A start may still be waiting for readiness; this action now owns the state.
    this.startToken++
    this.setState(DaemonState.STARTING, null)
   ...
   *
   * A crash is NOT restarted: `exited` is the signal an operator needs, and a
   * revive loop would hide a crash loop (an OOM-killed dsh comes straight back
   * to be killed again). Recovery is explicit — click 启动 in the portal.
   */
      ...
      ws.send(
        JSON.stringify({
          ...
          payload: {
            ...
            // Which control action produced the current state, so the portal can
            // say 启动中 / 关闭中 / 重启中 instead of a generic 处理中.
            daemonAction: this.lastAction || '',
  ...
  /**
   * Send the current state right now instead of waiting for the next tick.
   * Called from setState so 启动中 → 运行中 → 已关闭 arrive within milliseconds;
   * without it the portal can look frozen for up to HEARTBEAT_INTERVAL_MS (15s).
   */
  heartbeatNow() {
    if (typeof this.beat === 'function') this.beat()
  }
