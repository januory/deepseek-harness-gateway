...
面向公网部署的多租户**网关路由器**：把分布在各客户机上的 [DeepSeek Harness (dsh)](https://github.com/deepseek-ai/deepseek-harness) 实例，统一接入一条受管反向隧道。管理员审批机器入网并分配给普通用户，用户在网关 Web 门户上完整操控被分配机器的 dsh WebUI——客户机**零公网暴露**。
...
- **多租户网关** —— 唯一公网入口；租户、机器注册审批、用户分配、席位、审计都收口在网关。
...
2. 为要接入的租户签发配对码：
   ...
   ```
3. 在客户机上装好接入插件（见"安装"），打开 dsh 的 **设置 → 网关接入**，填入网关地址（`wss://<网关主机>/agent`）与配对码，点 **发起入网申请**。
