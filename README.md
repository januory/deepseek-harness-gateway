...
- **Multi-tenant gateway** — one public entry point; tenants, machine registration/approval, user assignment, seats, and audit all live at the gateway.
...
```
Public gateway (only exposed surface, multi-tenant)      Customer machine (zero inbound)

┌──────────────────────────────────────┐                   ┌──────────────────────────────────────┐
│Web portal / control plane / router   │                   │dsh-gateway-agent plugin              │
│register · tenants · assign · audit   │◄── wss outbound ──│(installed in customer dsh)           │
│                                      │                   │↓ loopback                            │
│                                      │                   │dsh web :3080                         │
└──────────────────────────────────────┘                   └──────────────────────────────────────┘
```
...
2. Issue a pairing code for the tenant to onboard:
   ```sh
   GATEWAY_PAIRING_CODES="<code>:<tenantId>" pnpm --filter dsh-gateway-server dev
   ```
3. On the customer machine, install the agent plugin (see Installation), then open the dsh **Settings → 网关接入** section, enter the gateway address (`wss://<gateway-host>/agent`) and the pairing code, and click **发起入网申请**.
